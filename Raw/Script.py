...
def generate_printable_report():
    ...
    if system_state['step'] > 1:
        past_wears = df['Tool wear [min]'].iloc[:system_state['step']].values
        df_sub = df.iloc[:system_state['step']]
        for i in range(1, len(past_wears)):
            if past_wears[i] < past_wears[i-1] - 50: 
                wear_val = past_wears[i-1]
                row_prev = df_sub.iloc[i-1]
                
                reason = 'Proactive'
                c = COLOR_SAFE
                for f_type in ['TWF', 'HDF', 'PWF', 'OSF', 'RNF']:
                    if row_prev[f_type] == 1:
                        reason = f_type
                        c = COLOR_DANGER
                        break 
                
                if wear_val >= 150:
                    swaps_eol_wear.append(wear_val)
                    swaps_eol_reason.append(reason)
                    swaps_eol_color.append(c)
                else:
                    if reason != 'Proactive':
                        premature_counts[reason] += 1
    ...
    legend_elements_swaps = [
        Patch(facecolor=COLOR_DANGER, label='Tool Broke'),
        Patch(facecolor=COLOR_SAFE, label='Swapped on Time')
    ]
...
fig.text(0.5, 0.02, '[SPACEBAR]: Pause/Resume Data Feed   |   [ENTER]: Generate Executive Report', ha='center', color=COLOR_WARN, fontsize=12, fontweight='bold')
...
2. Live Overstrain Analysis (Top Right):
Maps Force (Torque) vs Age (Wear). Safe Zone is boxed by limits (Curve & 200 min).

3. Historical Failures (Bottom):
- TWF (Tool Wear Failure): Tool physically snapped from age/over-usage.
- HDF (Heat Dissipation Failure): Machine overheated due to high speed/temp.
- PWF (Power Failure): Unsafe power drop or surge detected.
- OSF (Overstrain Failure): Mechanical force limits exceeded, breaking part.
- RNF (Random Failure): Unpredictable anomalies / Sensor ghosting.
...
order1_seq = ['Failure Threshold (200)', 'Uncertainty Zone', 'Mean Prediction', 'Actual Wear']
